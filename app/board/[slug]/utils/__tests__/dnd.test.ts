...
const clone = <T,>(value: T): T => structuredClone ? structuredClone(value) : JSON.parse(JSON.stringify(value));
...
describe("dnd utils", () => {
  ...
  it("returns null when the active card is missing", () => {
    const board = clone(baseBoard);
    const over = { id: "col-todo", data: { current: { columnId: "col-todo", sortable: { index: 0 } } } } as any;
